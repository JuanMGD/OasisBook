...
const DragDropImage = ({imagen, setImagen}) => {
    ...
    const ajustarImgURL = (image) => {
        return image.includes('cover') ? `http://localhost:8000/${image}` : image
    }
        ...
        <>
            ...
            {imagen 
                ...
                <div className={dragActive ? "drag-active dragImageContainer" : "dragImageContainer" } onClick={onButtonClick} >
                    <img className="image-preview" src={typeof imagen === 'object' ? URL.createObjectURL(imagen) : ajustarImgURL(imagen)} alt="imagen producto" onDragEnter={handleDrag} /> 
